...
void* mm_malloc (size_t size) {
  ...
  BlockInfo *firstFree = (BlockInfo *)searchFreeList(reqSize);
  size_t prevsize;

  //If firstFree == NULL, requestmorespace
  if(firstFree == NULL){

	  requestMoreSpace(reqSize);
	  firstFree = FREE_LIST_HEAD;
  }



  prevsize = SIZE(firstFree->sizeAndTags);

  BlockInfo *nextFree;
  BlockInfo *currentNextFree = firstFree->next;
  BlockInfo *currentPrevFree = firstFree->prev;
  //Allocate the whole block if prevsize-reqSize<MIN_BLOCK_SIZE
  if(prevsize-reqSize< MIN_BLOCK_SIZE){
	  removeFreeBlock(firstFree);
	  //may have problem here
	  firstFree->sizeAndTags = prevsize|TAG_USED;
	  size_t checkvalue = firstFree->sizeAndTags;

	  firstFree->sizeAndTags = checkvalue|TAG_PRECEDING_USED;
	  //checkvalue = firstFree->sizeAndTags;
	  size_t checkfollowingvalue = *((size_t *)UNSCALED_POINTER_ADD(firstFree,prevsize));

	  *((size_t *)UNSCALED_POINTER_ADD(firstFree,prevsize)) = checkfollowingvalue |TAG_PRECEDING_USED;


  }
  else{
	 //allocate a subblock
	  nextFree = (BlockInfo *)UNSCALED_POINTER_ADD(firstFree,reqSize);

	  nextFree->sizeAndTags = (prevsize-reqSize)|TAG_PRECEDING_USED;
	  ...
	  *((size_t*)UNSCALED_POINTER_ADD(nextFree,prevsize-reqSize-WORD_SIZE)) = (prevsize-reqSize)|TAG_PRECEDING_USED;
	  //size_t printvalue = (prevsize-reqSize)|TAG_PRECEDING_USED;

	  //Have already assigned the next free block we just need to change currentBlock
	  firstFree ->sizeAndTags = reqSize|TAG_USED;
	  firstFree ->sizeAndTags = (firstFree->sizeAndTags)|TAG_PRECEDING_USED;
  }
  ...
  return (void *)UNSCALED_POINTER_ADD(firstFree,WORD_SIZE);
}
...
void mm_free (void *ptr) {
	...
	size_t checkPrecedingUse;
	if(ptr == NULL){
		  return;
	  }
	checkPrecedingUse = blockInfo->sizeAndTags&TAG_PRECEDING_USED;
	payloadSize = SIZE(blockInfo->sizeAndTags);
	*((size_t*)UNSCALED_POINTER_ADD(blockInfo,payloadSize-WORD_SIZE)) = payloadSize|checkPrecedingUse;
	blockInfo->sizeAndTags = payloadSize|checkPrecedingUse;
	...
	//printf("followingBlock in mmfree address %p\n",followingBlock);
