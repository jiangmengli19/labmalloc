...
static void mark(void * ptr){
  ...
  //printf("find a pointer\n");


  if(((*blockHeader)&(BLOCK_MARKED))==BLOCK_MARKED){
	  return;
  }
  //printf("come to here\n");
  *blockHeader = (*blockHeader)|BLOCK_MARKED;
  size_t headsize = SIZE(*blockHeader)/WORD_SIZE;
  //printf("headsize %zu\n",headsize);
  for(int i = 0;i<=headsize-1;i++){
	  size_t *newptr = *(size_t *)((size_t *)ptr+i);
	  //printf("pointer %p\n",newptr);
	  mark(newptr);
  }
  return;
...
static void sweep(){
  ...
  while(curBlock < heapFooter){
      ...
      //void * payloadPtr = UNSCALED_POINTER_ADD(curBlock, WORD_SIZE);
      size_t allocated = sizeAndTags & TAG_USED;
      size_t blockmarked = sizeAndTags & BLOCK_MARKED;
      //printf("blockmark %zu\n",blockmarked);
      size_t blocksize = SIZE(sizeAndTags);
      if ((!blockmarked) && allocated){

        mm_free(UNSCALED_POINTER_ADD(curBlock,WORD_SIZE));
      }
      curBlock = (size_t *) UNSCALED_POINTER_ADD(curBlock, blocksize);
    }
  return;
  // ** YOUR CODE HERE **//
